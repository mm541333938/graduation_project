...
public class UserModel {
	...
	public void saveSql() {
		...
		connector.executeUpdate(
				...
				this.pwd, this.id, this.regDate, this.root);
		connector.executeUpdate("INSERT INTO `app_Info` (`user_id`, `name`, `image`) VALUES (?, ?, ?)", this.id,
				this.name, "image/profile.jpg");
	}
	...
	public String login() {
		...
		Boolean isCorrectPWD = connector.isExist("SELECT email, pwd, id, root FROM app_user WHERE email=? AND pwd=?",
				this.email, this.pwd);
		if (!isCorrectPWD) {
			return "密码有误";
		}
